...
class Pi0Sources(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sorted_labels = ['K^{*0}', '#barK^{*0}', 'K^{*-}', 'K^{*+}', 'c',
                              'K_{S}^{0}', 'g', 's', '#eta', '#omega', 'd', 'u', '#rho^{-}', '#rho^{+}']
        self.labels = {'c': 4, 'd': 1, 'g': 21, 'K_{S}^{0}': 310, '#omega': 223, '#rho^{-}': -213, '#barK^{*0}': -
                       313, '#rho^{+}': 213, 's': 3, 'u': 2, '#eta': 221, 'K^{*-}': -323, 'K^{*0}': 313, 'K^{*+}': 323}
        self.cut = 1e-2
    ...
    def testDifferentContributions(self):
        def contribution(contr_type='secondary'):
            ...
            hists = map(lambda x: self.read(
                '{0}_{1}'.format(filename, x), x, 999), labels)
            hists[0].logy = True
    ...
    def mostProbableBins(self, hist):
        ...
        bins = {int(nhist.GetBinCenter(i) - 0.5): nhist.GetBinContent(i)
                for i in range(1, nhist.GetNbinsX() + 1) if nhist.GetBinContent(i) > self.cut}
    ...
    def adjustBins(self, hist):
        nhist = ROOT.TH1F('{}_binned'.format(hist.GetName()),
                          hist.GetTitle(), len(self.labels), 0, len(self.labels))
