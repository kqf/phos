...
class TestMass(unittest.TestCase):
    """
        Use this code to determine the best matching
        parametrization for the area approximation
    """
    ...
    def setUp(self):
        ...
        self.analysis = Spectrum(
            self.input,
            label='fixed cb parameters',
            mode='q', options=Options(particle='eta', relaxedcb=True))
        self.mass, self.width = self.analysis.evaluate()[0:2]
        self.target = self.mass
        self.models = {
            "TMath::Exp([0] + [1] * x ) * [2] * x + [3]":
            [-0.9383877408174801, -7.313302813528194, -
                102.88494983600319, 0.5590698449654556],
            "[0] + 0 * [1]": [0.558684, 0]
        }
...
class TestWidth(TestMass):
    ...
    def setUp(self):
        ...
        self.models = {
            "TMath::Exp([0] + [1] * x ) * [2] * x + [3]":
            [0.2629952468139744, -2.331651575377617,
             -0.1217264515698897, 0.014533326264109624],
            "[0] + 0 * [1]":
            [0.012009189559948466, 0]
        }
...
class TestMassParameters(unittest.TestCase):
    ...
    def testMass(self):
        ...
        fitmass = fs.fit_quantity(
            mass, fs.mass_func, pars, fs.mass_names, 'mass')
        print fs.mass_pars
