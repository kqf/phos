...
class CheckProbability(test.check_default.CheckDefault):
    ...
    def setUp(self):
        ...
        npi0 = 20000
        self.hist = ROOT.TH1F('hGenerated', '"Gnerated" (exponential distribution) #pi^{0} energy spectrum N_{#pi^{0}} = %d; E, GeV' % npi0, 20, 0, 20)
        f1 = ROOT.TF1('pi0spectrum', 'TMath::Exp(-0.1 * x)',0, 20)
        self.hist.FillRandom('pi0spectrum', npi0)
    ...
    def testProbability(self):
        ...
        reco2 = self.reconstruct(self.hist, 'w1 * w2', lambda x, y: 1. /(x * y))
        reco3 = self.reconstruct(self.hist, '(w1 + w2)/2', lambda x, y: 1./ ((x + y)/2.) )
