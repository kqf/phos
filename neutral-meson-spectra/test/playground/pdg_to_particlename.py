...
class Efficiency(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.particles = [1, 2, 3, 4, -313, -213, 221, 323, 21, 310, 313, 223, -323, 213]
    ...
    def testExtract(self):
        ...
        def get_codes(name, label):
            hist = read_histogram(ifile, 'MCStudyOnlyTender', name, label = label)
            codes = map(int, [hist.GetBinCenter(i) - 0.5 for i in range(1, hist.GetNbinsX() + 1) if hist.GetBinContent(i) > 0])
            return codes
