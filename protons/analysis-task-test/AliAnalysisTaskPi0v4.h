...
{
  EventProperties(Bool_t a, Bool_t b, Bool_t c, Bool_t d, Int_t z):
    eventVtxExist(a),
    eventVtxZ10cm(b),
    eventPileup(c),
    eventV0AND(d),
  ...
  Bool_t eventVtxExist;
  Bool_t eventVtxZ10cm;
  Bool_t eventPileup;
  Bool_t eventV0AND;
  Int_t zvtx;
};
...
private:
  void FillCellsHistograms(AliVCaloCells * cells);
  void FillClusterHistograms(AliVEvent * event, Double_t vtx0[3]);
  void SelectPhotons(AliVEvent * event, TClonesArray * fPHOSEvent, Double_t vtxBest[3]);
  void FillCombinations(AliCaloPhoton * ph1, AliCaloPhoton * ph2, const EventProperties & eprop, const char * suff = "");
  EventProperties SelectEvent(AliVEvent * event, Double_t vtxBest[3]);
