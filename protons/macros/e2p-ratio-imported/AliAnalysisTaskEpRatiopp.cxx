...
#include "AliAnalysisTaskEpRatiopp.h"
...
// Date   : 04.07.2013
// with Absolute E calibration on cluster E :08-07-16
ClassImp(AliAnalysisTaskEpRatiopp)
//________________________________________________________________________
AliAnalysisTaskEpRatiopp::AliAnalysisTaskEpRatiopp(const char *name) :  AliAnalysisTaskSE(name),
  fRunNumber(-999),
  fOutputContainer(0),
  fPIDResponse(0), // Yuri
  fPHOSGeo(0)
// Output slots #0 write into a TH1 container
{
  ...
  // Absolute recalibration for LHC10X. Use SetRecalib(mod,recalib) to change it
  // fRecalib[0] = 0.9822696;
  // fRecalib[1] = 0.9861288;
  // fRecalib[2] = 1.0072;
  
  // Date:10-07-16 Absolute recalibration for LHC10X. Use SetRecalib(mod,recalib) to change it
  fRecalib[0] = 0.9694629;
  fRecalib[1] = 0.9771681;
  fRecalib[2] = 0.9980281;
  
}
//________________________________________________________________________
void AliAnalysisTaskEpRatiopp::UserCreateOutputObjects()
{
  // Create histograms
  // Called once
...
//________________________________________________________________________
void AliAnalysisTaskEpRatiopp::UserExec(Option_t *) 
{
  ...
  for(Int_t iClu=0; iClu<kNumberOfCaloClusters; iClu++)   {
    ...
    FillHistogram("h_EClus_NCell",energy,c1->GetNCells());
    FillHistogram("h_ECluster",energy);
    FillHistogram("h_NCells",c1->GetNCells());

    if( c1->GetNCells()<3 ) continue; 

    FillHistogram("h_EClusEvent",energy);
    
    if( mod1 == 1 ){
      inMod1++; nPHOSCluster++;
      FillHistogram("h_EClusEventmod1",energy);
    }
    else if( mod1 == 2 ){
      inMod2++; nPHOSCluster++;
      FillHistogram("h_EClusEventmod2",energy);
    }
    else if( mod1 == 3 ){
      inMod3++; nPHOSCluster++;
      FillHistogram("h_EClusEventmod3",energy);
    }
    ...
    AliVTrack* esdTrack = dynamic_cast<AliVTrack*> (c1->GetTrackMatched(0));//only for AODs
    ...
    if(fPIDResponse) {
      ...
      if(energy<0.3) continue;  // MIP & noise cut
      
      // =============================================================================
      FillHistogram("hEp",energy/esdTrack->P(),esdTrack->Pt());
      FillHistogram("h_dedx_PHOS",esdTrack->P(),esdTrack->GetTPCsignal());
      
      if( mod1 == 1 ){
	FillHistogram("hEp_mod1",energy/esdTrack->P(),esdTrack->Pt());
	FillHistogram("h_ClusNXZM1",cellX,cellZ,1.);
      }
      else if( mod1 == 2 ){
	FillHistogram("hEp_mod2",energy/esdTrack->P(),esdTrack->Pt());
	FillHistogram("h_ClusNXZM2",cellX,cellZ,1.);
      }
      else if( mod1 == 3 ){
	FillHistogram("hEp_mod3",energy/esdTrack->P(),esdTrack->Pt());
	...
	FillHistogram("h_dedx_proton_PHOS",esdTrack->P(),esdTrack->GetTPCsignal());
      ...
      if (pidElectron){
	FillHistogram("hEp_ele",energy/esdTrack->P(),esdTrack->Pt());
        FillHistogram("hEp_ele2",energy/esdTrack->P(),1/esdTrack->P());
	FillHistogram("h_dedx_ele_PHOS",esdTrack->P(),esdTrack->GetTPCsignal());

	// =============================================================================

	if (charge>0) FillHistogram("hEp_positron",energy/esdTrack->P(),esdTrack->Pt());
        if (charge<0) FillHistogram("hEp_electron",energy/esdTrack->P(),esdTrack->Pt());

	if( mod1 == 1 ){
	  FillHistogram("h_ClusNXZM1_ele",cellX,cellZ,1.);
	  FillHistogram("hEp_ele_mod1",energy/esdTrack->P(),esdTrack->Pt());
	}
	else if( mod1 == 2 ){
	  FillHistogram("h_ClusNXZM2_ele",cellX,cellZ,1.);
	  FillHistogram("hEp_ele_mod2",energy/esdTrack->P(),esdTrack->Pt());
	}
	else if( mod1 == 3 ){
	  FillHistogram("h_ClusNXZM3_ele",cellX,cellZ,1.);
	  FillHistogram("hEp_ele_mod3",energy/esdTrack->P(),esdTrack->Pt());
	}
      }
      else{
	FillHistogram("hEp_other",energy/esdTrack->P(),esdTrack->Pt());
	FillHistogram("h_dedx_other_PHOS",esdTrack->P(),esdTrack->GetTPCsignal());

	if( mod1 == 1 ){
	  FillHistogram("hEp_other_mod1",energy/esdTrack->P(),esdTrack->Pt());
	}
	else if( mod1 == 2 ){
	  FillHistogram("hEp_other_mod2",energy/esdTrack->P(),esdTrack->Pt());
	}
	else if( mod1 == 3 ){
	  FillHistogram("hEp_other_mod3",energy/esdTrack->P(),esdTrack->Pt());
	}

      }

      if (pidHadron){
	FillHistogram("hEp_had",energy/esdTrack->P(),esdTrack->Pt());
	FillHistogram("hEp_had2",energy/esdTrack->P(),1/esdTrack->P());
	...
	if( mod1 == 1 ){
	  ...
	  FillHistogram("hEp_had_mod1",energy/esdTrack->P(),esdTrack->Pt());
	}
	else if( mod1 == 2 ){
	  FillHistogram("h_ClusNXZM2_had",cellX,cellZ,1.);
	  FillHistogram("hEp_had_mod2",energy/esdTrack->P(),esdTrack->Pt());
	}
	else if( mod1 == 3 ){
	  FillHistogram("h_ClusNXZM3_had",cellX,cellZ,1.);
	  FillHistogram("hEp_had_mod3",energy/esdTrack->P(),esdTrack->Pt());
	}
      }
...
void AliAnalysisTaskEpRatiopp::FillHistogram(const char * key,Double_t x)const{
  //FillHistogram
...
//_____________________________________________________________________________
void AliAnalysisTaskEpRatiopp::FillHistogram(const char * key,Double_t x,Double_t y)const{
  //FillHistogram
...
//_____________________________________________________________________________
void AliAnalysisTaskEpRatiopp::SetGeometry()
{
  //Init geometry.
...
//_____________________________________________________________________________
void AliAnalysisTaskEpRatiopp::FillHistogram(const char * key,Double_t x,Double_t y, Double_t z) const{
