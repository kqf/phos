AliAnalysisGrid * CreatePlugin(TString pluginmode = "test", TString runperiod)
{
	...
	plugin->SetGridDataDir("/alice/data/2016/" + runperiod);
	...
	if (runperiod == "LHC16g")
	{
		...
		Int_t RunNumbers[] =
		{
			254128, 254147, 254148, 254149, 254174, 254175, 254178, 254193, 254196, 254199, 
			254204, 254205, 254214, 254223, 254241, 254270, 254303, 254304, 254330, 254331, 
			254332
		};
	}
