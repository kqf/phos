...
def get_my_list(filename = 'AnalysisResults.root'):
    ...
    ROOT.gROOT.cd()
    mlist = [k.ReadObj().Clone() for k in mlist] 
    return mlist
...
def compare_lists_of_histograms(l1, l2, ignore = []):
    ...
    for h in l1: 
        ...
        mean = h.GetMean() - candidate.GetMean()
        rms = h.GetRMS() - candidate.GetRMS()
        if not isclose(1., percentile) and not h.GetName() in ignore: 
            print bcolors.WARNING + 'Rate of change of %s%s%s is' % (bcolors.OKGREEN, h.GetName(), bcolors.WARNING), percentile, mean, rms, bcolors.ENDC
...
def compare_histograms():
    ...
    mlist2 = get_my_list('ProductionQA.hist.root')
