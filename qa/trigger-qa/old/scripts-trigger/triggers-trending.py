...
class TrendDrawer(object):
    ...
    def form_trending_plots(self, sm, runs):
        def namef(x, y):
            return 'h_%s_TRU_%s' % (str(x), str(y))
        nbins, title = len(runs), self.quantity.title
        hists = [ROOT.TH1F(namef(sm, i + 1), title % sm, nbins, 0, nbins)
                 for i in range(self.ntrus)]
        for i, h in enumerate(hists):
            h.label = 'TRU ' + str(i + 1)  # custom legend label
        ...
        for ibin, run in enumerate(sorted(runs)):
            for itru, tru in enumerate(runs[run]):
                ...
                if ibin == 0:
                    # This is completely different thing: makes vertical bins
                    hists[itru].LabelsOption('v')
    ...
    def draw_trending_plots(self, data):
        ...
        for sm in sorted(data):
            hists = [h for h in self.form_trending_plots(
                sm, data[sm]) if h.GetMaximum() > 0]
            if not hists:
                print 'No data available for', sm
                continue

            hists[0].SetAxisRange(0.01, 1.01 * max(h.GetMaximum()
                                                   for h in hists), 'Y')
            ...
            canvas.SetGridy()
            canvas.Update()
            canvas.SaveAs(self.path + '/images/' +
                          self.quantity.oname + '_' + sm + '.pdf')
...
class Quantity(object):
    ...
    def extract_modules(self, fname):
        ...
        run_numbers = sorted(set([r.GetName().split('M')[0]
                                  for r in ifile.GetListOfKeys()]))
        ...
        def get_ratio(x):
            return [h for h in x if 'Ratio' in h.GetName()]
        raw = {sm: {nrun: get_ratio(ifile.Get(nrun + sm))
                    for nrun in run_numbers} for sm in supermods}
        data = {sm: {nrun: self.extract_run(
            raw[sm][nrun]) for nrun in raw[sm]} for sm in raw}
        return data


def tru_coordinates(itru):
    assert (itru >= 1) and (itru <= 8), \
...
class QuantityChannels(Quantity):
    ...
    def extract_run(self, perrun):
        res = {str(itru): self.extract_tru(itru, perrun)
               for itru in range(1, 9)}
        # print res
        # nicelydraw(perrun)
        return res
    ...
    def extract_modules(self, fname):
        ...
        def get_hist(run, sm):
            return ilist.FindObject(run).FindObject(self.histname + sm)
        raw = {sm: {run: get_hist(run, sm)
                    for run in run_numbers} for sm in supermods}

        def get_nevents(run):
            runhist = ilist.FindObject(run)
            return runhist.FindObject(self.nevents).GetBinContent(1)
        ...
        data = {sm: {run: self.extract_run(
            raw[sm][run]) for run in sorted(raw[sm])} for sm in raw}
        return data


def integral(h, x, y, norm):
    val_err = ROOT.Double(0)
    val, norm_err = h.IntegralAndError(
        x[0], x[-1], y[0], y[-1], val_err), sqrt(norm)
    res, res_err = val / \
        norm, sqrt((val_err / norm) ** 2 + (norm_err * val / norm / norm) ** 2)
    print res, res_err
    return res, res_err
...
def main():
    ...
    title = "Triggered clusters / all clusters in %s in E > 4.5 GeV region;;"\
        "Triggered clusters / all clusters"
    ratio = Quantity(title)
    TrendDrawer(path, 'ratio.root', ratio)

    trigfile = 'TriggerQASingle.root'

    def draw_quantity(q):
        return TrendDrawer(path, trigfile, q)

    cluster_types = {'': '', 'matching cluters with E > 2 GeV': 'Clu'}
    for tp in cluster_types:
        cltype = cluster_types[tp]

        ctitle = "Probability for TRUs in %s to be fired, " + \
            tp + " ;; N_{Fired channels} / N_{kINT7}"
        probability = QuantityChannels(ctitle, cluster=cltype, cellf=integral)
        draw_quantity(probability)

        ptitle = "Number of TRU channels fired in %s"\
            " {};; # of TRU channels".format(tp)
