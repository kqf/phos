...
class TestRatio(TestImages):
	...
	def save_histogram(self, filename, histnames):
		...
		for i, histname in enumerate(histnames):
			f = ROOT.TF1('f%d' % i, 'TMath::Exp(- %0.2f * x) + (1 - %d) * TMath::Exp(-1 * (x - 9) * (x - 9))' % (i / 10., i), 0, 20)
			histogram = ROOT.TH1F(histname, "Testing %dth iteration ..." % i, 1000, 0, 20)
